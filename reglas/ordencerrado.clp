...
(defrule en-circulo-mar
    ...
    =>
    (?*list* add "Los soldados se mueven con paso vivo y forman un semicírculo.")
    (?*list* add "El primero que llega se coloca inmóvil nueve pasos delante del superior que manda.")
    (?*list* add "Los demás conforme llegan se colocan alternativamente a derecha e izquierda de este formando el semicírculo.")
    (?*list* add "todos los soldados permanecen inmóviles hasta que el superior mande A discreción.")
)
...
(defrule a-formar-en-linea-mar
    ...
    (?*list* add "El segundo hombre forma un asegunda fila colocándose ochenta centímetros de distancia del primer hombre de la primera fila.")
    (?*list* add "Los demás hombres se colocan a la izquierda del primer y segundo hombre, de la primera y segunda fila respectivamente.")
)
...
(defrule a-formar-de-uno-mar
    ...
    (?*list* add "Los demás hombres conforme llegan se sitúan unos detrás de otros con ochenta centímetros de distancia.")
    (?*list* add "A la voz de Discreción el último hombre de la cola pasa a discreción.")
)
...
(defrule a-formar-en-columnas-por-hilera-mar
    ...
    (?*list* add "Los demás hombres conforme llegan se colocan detrás de ambas columnas con ochenta centímetros de distancia.")
    (?*list* add "a la voz de discreción el último hombre de la columna izquierda pasa a discreción.")
)
...
(defrule numerarse
    ...
    (?*list* add "Los otros soldados pasan sucesivamente el número que le corresponde.")
)
...
(defrule por-sus-funciones-nombrarse
    ...
    (?*list* add "O Los soldados pasan la voz indicando en voz alta las funciones que desempeñan en la escuadra seguidamente del número que tienen en ella.")
)
...
(defrule el-primero-tres-al-fren
    ...
    (?*list* add "Corrige su posición (posición firmes) Permaneciendo inmóvil con la vista al frente hasta que el comandante de escuadra vuelve a mandar diciendo: 'El siguiente alinee-¡ar!'.")
    (?*list* add "El segundo soldado dirige la vista hacia la cabeza del primer hombre.")
    (?*list* add "a la ejecutiva de “ar”.")
    (?*list* add "Se coloca rápidamente a la izquierda del primer hombre.")
    (?*list* add "Los demás hombres hacen lo mismo.")
)
...
(defrule soldado-x-tres-al-fren
    ...
    (?*list* add "Corrige su posición (posición firmes) Permaneciendo inmóvil con la vista al frente hasta que el comandante de escuadra vuelve a mandar diciendo: 'El siguiente vista a la izquier alinee-¡ar!'.")
    (?*list* add "El soldado de la derecha dirige la vista hacia la cabeza del primer hombre")
    (?*list* add "a la ejecutiva de 'ar'.")
    (?*list* add "Se coloca rápidamente a la derecha del primer hombre.")
    (?*list* add "Los demás hombres hacen lo mismo.")
)
